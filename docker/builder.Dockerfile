...
RUN --mount=type=bind,target=/root,rw cd /root && make -C tools/framework/examples/${VENDER} && cp tools/framework/examples/${VENDER}/libtai-${VENDER}.so `cat /tmp/lib`/
RUN cd `cat /tmp/lib` && ln -s libtai-${VENDER}.so libtai.so
